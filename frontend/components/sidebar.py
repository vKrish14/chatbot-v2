...
def render_sidebar():
    with st.sidebar:
        ...
        if "memory_stats" in st.session_state and st.session_state["memory_stats"]:
            ...
            st.caption(f"Retained Context: {stats.get('retained_messages', 0)} / {stats.get('total_messages', 0)}")
            st.caption(f"Estimated Tokens: {stats.get('estimated_tokens', 0)}")
        
        if st.button("Purge Context", use_container_width=True):
        ...
        search_strategy = st.selectbox("Search Strategy", ["similarity", "mmr", "hybrid"], index=2)
